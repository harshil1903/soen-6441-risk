...
import static com.risk.gameutils.SaveGame.*;
 ...
 * @author Chirag
 */
public class AssignCountriesGame {
    ...
    public static void assignRandomCountries(ArrayList<Player> p_list, ArrayList<Country> p_countries,
                                             HashMap<Integer, Integer> p_hashMap,Map d_Map) {
     ...
     */
    public static void assignCountries(ArrayList<Player> d_playerListGame,Map d_Map) throws InvalidMapException {
        ArrayList<Country> l_countries = fillCountryList(d_Map);
        HashMap<Integer, Integer> l_map = new HashMap<>();
        for (int i = 0; i < d_playerListGame.size(); i++) {
            l_map.put(i, l_countries.size() / d_playerListGame.size());
        }
        assignRandomCountries(d_playerListGame, l_countries, l_map,d_Map);
